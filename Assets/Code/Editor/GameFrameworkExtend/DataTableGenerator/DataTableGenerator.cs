...
namespace UGHGame.GameEditor
{
    ...
    {
        private const string DataTablePath = "Assets/HotfixAssets/DataTables";
        private const string CSharpCodePath = "Assets/Code/BuiltinRuntime/DataTable";
        private const string CSharpHotfixCodePath = "Assets/Code/HotfixLogic/DataTable";
        private const string CSharpCodeTemplateFileName = "Assets/Code/Editor/GameFrameworkExtend/DataTableGenerator/DataTableCodeTemplate/DataTableCodeTemplate.txt";
        ...
        public static void GenerateHotfixCodeFile(DataTableProcessor dataTableProcessor , string dataTableName)
        {
            dataTableProcessor.SetCodeTemplate(CSharpCodeTemplateFileName , Encoding.UTF8);
            dataTableProcessor.SetCodeGenerator(DataTableCodeGenerator);

            string csharpCodeFileName = Utility.Path.GetRegularPath(Path.Combine(CSharpHotfixCodePath , "DR" + dataTableName + ".cs"));
            if(!dataTableProcessor.GenerateCodeFile(csharpCodeFileName , Encoding.UTF8 , dataTableName , isHotfixCSharp: true) && File.Exists(csharpCodeFileName))
            {
                File.Delete(csharpCodeFileName);
            }
        }

        private static void DataTableCodeGenerator(DataTableProcessor dataTableProcessor , StringBuilder codeContent , object userData , bool isHotfixCSharp)
        {
            string dataTableName = (string)userData;

            //codeContent.Replace("__DATA_TABLE_CREATE_TIME__", DateTime.UtcNow.ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss.fff"));
            codeContent.Replace("__DATA_TABLE_NAME_SPACE__" , isHotfixCSharp ? "MyGame.Hotfix" : "MyGame");
