...
using UnityEditor;
using UnityEngine;
using static UGHGame.BuiltinRuntime.ComponentAutoBindTool;
...
namespace UGHGame.GameEditor
{
    ...
    /// ui自动绑定帮助器
    /// </summary>
    public class UIAutoBindRuleHelper:Editor
    {
        ...
        public override void OnInspectorGUI( )
        {
            ...
            {
                DrawButton("排序" , Sort);
                DrawButton("全部删除" , RemoveAll);
                DrawButton("删除空引用" , RemoveNull);
                DrawButton("自动绑定组件" , AutoBindComponent);
            }
        ...
        private void DrawButton(string buttonName , Action action)
        {
        ...
        private bool IsValidBind(Transform target , List<string> filedNames , List<string> componentTypeNames)
        {
            ...
            for(int i = 0; i < strArray.Length - 1; i++)
            {
                ...
                if(m_PrefixesDict.TryGetValue(str , out string prefix))
                {
                    filedNames.Add($"{str}_{fileName}");
                    componentTypeNames.Add(prefix);
                }
                else
                {
                    Debug.LogError($"{target.name}的命名中{str}不存在对应的组件");
                    return false;
                }
            }
            return true;
