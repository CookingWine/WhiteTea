...
{
    [CustomEditor(typeof(BuiltinComponentAutoBindTool))]
    public class BuiltinComponentAutoBindToolInspector:Editor
    {
        private SerializedProperty m_BindDatas;
        private SerializedProperty ScriptSpaceName;
        private SerializedProperty ScriptTypeName;
        ...
        private void OnEnable( )
        {
            ...
            ScriptSpaceName = serializedObject.FindProperty("ScriptSpaceName");
            ScriptTypeName = serializedObject.FindProperty("ScriptTypeName");
        ...
        public override void OnInspectorGUI( )
        {
            ...
            EditorGUILayout.BeginHorizontal( );
            {
                if(GUILayout.Button($"绑定‘{ScriptSpaceName.stringValue}.{ScriptTypeName.stringValue}'"))
                {
                    AutoBindScripts(ScriptSpaceName.stringValue , ScriptTypeName.stringValue);
                }
            }
            EditorGUILayout.EndHorizontal( );
            DrawKvData( );
        ...
        private void AutoBindScripts(string spaceName , string scriptName)
        {
            Assembly[] assemblies = AppDomain.CurrentDomain.GetAssemblies( );
            foreach(var item in assemblies)
            {
                if(item.FullName.Contains(spaceName))
                {
                    //Debug.Log(item);
                    foreach(var type in item.GetTypes( ))
                    {
                        //Debug.Log(type.Name);
                        if(type.Name.Contains(scriptName))
                        {
                            BuiltinComponentAutoBindTool target = (BuiltinComponentAutoBindTool)this.target;
                            target.gameObject.GetOrAddComponent(type);
                        }
                    }
                }
            }
        ...
        {
            m_BindDatas.ClearArray( );
            Transform[] childs = ( (BuiltinComponentAutoBindTool)target ).gameObject.GetComponentsInChildren<Transform>(true);
